...
import { RecommendedNotice, getRecommendedNotice, clearRecommendedNotice } from '../../services/versionGateService';
...
export default function CustomerHomeScreen() {
    ...
    const [recommendedNotice, setRecommendedNotice] = useState<RecommendedNotice | null>(null);
    ...
    useFocusEffect(
        React.useCallback(() => {
            ...
            const loadNotice = async () => {
                const notice = await getRecommendedNotice();
                setRecommendedNotice(notice);
            };
            loadNotice();
        }, [navigation])
    );
    ...
    const handleRecommendedUpdate = async () => {
        if (recommendedNotice?.storeUrl) {
            const supported = await Linking.canOpenURL(recommendedNotice.storeUrl);
            if (supported) {
                await Linking.openURL(recommendedNotice.storeUrl);
            } else {
                Alert.alert('Update', 'Cannot open the store link.');
            }
        }
    };

    const dismissRecommended = async () => {
    ...
    return (
        <View style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <SafeAreaView style={styles.overlayContainer} pointerEvents="box-none">
                {recommendedNotice && (
                    <View style={[styles.updateBanner, { backgroundColor: colors.surface, borderColor: colors.border, shadowColor: colors.shadow }] }>
                        <View style={{ flex: 1 }}>
                            <Text variant="h3" style={{ color: colors.textPrimary, marginBottom: 4 }}>Update available</Text>
                            <Text variant="body" style={{ color: colors.textSecondary }}>{recommendedNotice.message || 'A new version of the app is available.'}</Text>
                        </View>
                        <View style={styles.updateBannerActions}>
                            <TouchableOpacity style={[styles.updateBannerButton, { backgroundColor: colors.primary }]} onPress={handleRecommendedUpdate}>
                                <Text variant="caption" weight="bold" style={{ color: '#fff' }}>Update</Text>
                            </TouchableOpacity>
                            <TouchableOpacity style={[styles.updateBannerButton, { backgroundColor: 'transparent', borderColor: colors.border, borderWidth: 1 }]} onPress={dismissRecommended}>
                                <Text variant="caption" weight="bold" style={{ color: colors.textSecondary }}>Dismiss</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                )}
